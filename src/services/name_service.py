...
class NameService:
    ...
    async def get_countries_by_name(
        self, name: str, days_threshold: int = 1
    ) -> List[NameCountryResponse]:
        try:
            name_record = self.name_repo.get_or_create_name(name)
            cached_countries = self.name_repo.get_name_countries(
                name_record.id, days_threshold
            )
            if cached_countries:
                return [
                    NameCountryResponse(
            ...
            for country_data in response["country"]:
                ...
                if not country:
                    country_info = await self.rest_countries_client.get_country_by_code(
                        country_code
                    )
                    if not country_info:
                        raise APIError(
                            f"Failed to fetch country data for code {country_code}"
                        )
                    if isinstance(country_info, list) and country_info:
                        country_info = country_info[0]
                    rest_country = RestCountryResponse(
            ...
            handle_api_error(
                APIError(f"Unexpected error while processing name {name}: {str(e)}")
            )

    async def get_popular_names_by_country(
        self, country_code: str
    ) -> List[PopularNameResponse]:
        try:
            ...
            popular_names = self.name_repo.get_popular_names_by_country(
            ...
            return [
                PopularNameResponse(
                    name=name.name, count_of_requests=name.count_of_requests
                )
            ...
            raise APIError(
                f"Unexpected error while fetching popular names for country {country_code}: {str(e)}"
            )
