...
def mock_auth_settings(mocker):
    ...
    mocker.patch("src.config.auth_settings.auth_settings.ACCESS_TOKEN_EXPIRE_MINUTES", 30)
...
async def test_register_success(auth_service, mocker):
    user_data = UserCreate(username="newuser", email="new@example.com", password="password")
    mocker.patch.object(auth_service.user_repo, "get_user_by_username", return_value=None)
    mocker.patch.object(auth_service.user_repo, "create_user", return_value=MagicMock(username="newuser"))
...
async def test_login_success(auth_service, mocker):
    ...
    mock_user = MagicMock(username="testuser", hashed_password=auth_service.user_repo.pwd_context.hash("password"))
    mocker.patch.object(auth_service.user_repo, "get_user_by_username", return_value=mock_user)
...
async def test_auth_get_current_user_success(mock_db, mock_auth_settings, mocker):
    ...
    mocker.patch("src.repositories.user_repository.UserRepository.get_user_by_username",
